...
class LRFinder(object):
    ...
    def _train_batch(self, iter_wrapper, accumulation_steps):
        ...
        for i in range(accumulation_steps):
            ...
            pred_mask, pred_depth = torch.split(self.model(inputs), 1,dim=1)
            #mask_loss,depth_loss = self.criterion((labels[0],pred_mask), (labels[1],pred_depth))
            mask_loss = mmdlosses.construct_criterion((pred_mask, labels[0]), self.criterion['mask'])
            depth_loss = mmdlosses.construct_criterion((pred_depth, labels[1]), self.criterion['depth'])
...
class DataLoaderIterWrapper(object):
    ...
    def __next__(self):
        ...
        try:
            ...
            #labels = (dataset['output'][0], dataset['output'][1])
